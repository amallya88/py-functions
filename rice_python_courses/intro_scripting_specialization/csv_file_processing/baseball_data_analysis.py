...
def filter_by_year(statistics, year, yearid):
    ...
    Inputs:
      ...
      year       - Year (integer) to filter by
      yearid     - Year ID field in statistics
    Outputs:
      Returns a list of batting statistics dictionaries that
      are from the input year.
    """
    filt_statistics = [dict for dict in statistics if int(dict[yearid]) == year]
    return filt_statistics
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    players_for_formula = [(player_dict[pid_field], formula(info, player_dict))
                           for player_dict in statistics]
    filt_statistics = sorted(players_for_formula, key=lambda x: x[1], reverse=True)
    return filt_statistics[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    statistics = read_csv_as_nested_dict(info["masterfile"], info["playerid"],
                                         info["separator"], info["quote"])
    output_str_lst = []
    for player in top_ids_and_stats:
        stat = player[1]
        pid = player[0]
        output_str_lst.append("{:.3f} --- {} {}".format(stat,
                                                        statistics[pid].get(info["firstname"]),
                                                        statistics[pid].get(info["lastname"])))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    top_stats = top_player_ids(info,
                               filter_by_year(statistics, year, info["yearid"]),
                               formula,
                               numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for player_stats in statistics:
        ...
        if pid not in agg_stats:
            agg_stats[pid] = {playerid: pid}
        for field in fields:
            agg_stats[pid][field] = agg_stats[pid].get(field, 0) + float(player_stats.get(field))
    return agg_stats
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    Inputs:
      ...
      numplayers  - Number of top players to return
      Outputs:
          Returns a list of strings for the top numplayers according to the given formula
          for the span of their whole career
    """
    statistics_dict_lst = read_csv_as_list_dict(info["battingfile"],
                                                info["separator"],
                                                info["quote"])

    career_agg_stats = aggregate_by_player_id(statistics_dict_lst,
                                              info["playerid"],
                                              info["battingfields"])

    top_ids_and_stats = top_player_ids(info, career_agg_stats.values(), formula, numplayers)
