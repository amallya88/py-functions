...
def initialize_db(db_cur, tableName, db_schema, dropTableEn, db_options=[]):
    ...
    dropTableEn: If True drop existing table, otherwise create if not exist
    db_options: list of options e.g. specifying composite key UNIQUE(fieldA, fieldB) etc
    """
    if dropTableEn is True:
        db_cur.execute('DROP TABLE IF EXISTS {table}'.format(table=tableName))
        schema_str = "CREATE TABLE {table} ({fields}{options})"
    else:
        schema_str = "CREATE TABLE IF NOT EXISTS {table} ({fields})"

    fields = ['{} {}'.format(field['field_name'], field['field_type']) for field in db_schema]
    options = ''
    if len(db_options) > 0:
        options = "," + ",".join(db_options)

    schema_str = schema_str.format(table=tableName, fields=",".join(fields), options=options)
