...
class WOFHumanPlayer(WOFPlayer):
    """ A human player of the WOF game """

    def getMove(self, category, obscuredPhrase, guessed):
        board = showBoard(category, obscuredPhrase, guessed)
        prompt = """
        {} has ${}\n
        {}\n
        Guess a letter, phrase, or type 'exit' or 'pass'""".format(self.name, self.prizeMoney, board)
        guess = input(prompt)
        return guess
...
class WOFComputerPlayer(WOFPlayer):
    """ A computer player of WOF game"""

    SORTED_FREQUENCIES = 'ZQXJKVBPYGFWMUCLDRHSNIOATE'  # a list of English characters sorted from least frequent

    # ('Z') to most frequent ('E')
    ...
    def smartCoinFlip(self):
        """ choose rand between 1 and 10, if rand > self.difficult return True """
        return True if (random.randint(1, 10) > self.difficulty) else False

    def getPossibleLetters(self, guessed):
        """ guessed are letters that have already been guessed
        return a list of letters that are still available to be guessed """
        return [letter for letter in WOFComputerPlayer.SORTED_FREQUENCIES if letter not in guessed]

    def getMove(self, category, obscuredPhrase, guessed):
        """ Make a good or bad move based on random variable """
        move_quality = self.smartCoinFlip()
        guesses_left = ""
        if (sorted(VOWELS) == sorted(guesses_left)) and self.prizeMoney < VOWEL_COST or len(guessed) == len(LETTERS):
            return 'pass'
        else:
            if self.prizeMoney >= VOWEL_COST:
                guesses_left = self.getPossibleLetters(guessed)
            else:
                guesses_left = [lettr for lettr in self.getPossibleLetters(guessed) if lettr not in VOWELS]

            return guesses_left[0] if move_quality else random.choice(guesses_left)
